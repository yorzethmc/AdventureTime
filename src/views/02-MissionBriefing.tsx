...
const MissionBriefing = ({ onAccept, onReject }: Props) => {
  ...
  const handleEvade = useCallback(() => {
    ...
    if (newAttempts >= 9) {
      // On the 9th attempt, return button to normal position
      setBtnPosition(null);
      setIsEvading(false);
    } else {
      moveButton();
  ...
  const handleRejectHover = () => {
    // Only evade on hover for desktop; mobile uses click/touch
    if (rejectAttempts < 9) {
      handleEvade();
    }
  };
  ...
  return (
    <div className="rpg-panel fade-in">
      ...
      <div className="text-pixel mb-3" style={{ fontSize: '0.8rem', lineHeight: '1.8' }}>
        <p>Hay misiones que no aparecen todos los días.</p>
        <br />
        <p>Esta fue preparada con cuidado, un poco de magia retro y una intención muy simple: invitarte a una cita diferente.</p>
        <br />
        <p>Si aceptás, vas a elegir el camino, la hora, el destino y el combustible de esta aventura.</p>
      </div>
